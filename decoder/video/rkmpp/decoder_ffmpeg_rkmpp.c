...
int init_rkmpp_decoder(rkmppDecoder *dec, AVCodecParameters *par) {
    ...
    // Select the appropriate RKMPP decoder based on codec ID
    ...
    if ((ret = av_hwdevice_ctx_create(&hw_device_ctx, AV_HWDEVICE_TYPE_DRM,
        NULL, NULL, 0)) < 0) {
        return ret;
    }
    dec->ctxt->hw_device_ctx = av_buffer_ref(hw_device_ctx);
...
int decode_rkmpp_packet(rkmppDecoder *dec, uint8_t *buffer) {
    ...
    av_log(NULL, AV_LOG_WARNING, "receive frame ret: %d\n", ret);
    if (ret < 0) {
